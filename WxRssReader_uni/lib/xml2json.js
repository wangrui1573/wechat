(function (root) {
    ...
    Parser.prototype.readNode = function (sibling) {
        ...
        var text;
        var nextTest = /^[\s\n]*<(?!\/|!\[CDATA\[)/;
        var openTagStop = this.xmlString.indexOf('>', this.cursor);
        var openTag = this.readTag(this.xmlString.substring(this.cursor, openTagStop));
        Object.assign(node, openTag.nodeAttr);
        parent = {
            [openTag.nodeName]: node
        };
    ...
    Parser.prototype.readText = function (nodeName) {
        ...
        var end = this.xmlString.indexOf('</' + nodeName + '>', start);
        var cdStart = -1;
        var cdEnd;
        if (end < 0) {
            return text;
